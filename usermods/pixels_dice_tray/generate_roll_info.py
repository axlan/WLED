    ...
    for line in entry[1].splitlines():
      if line.startswith('$'):
        ...
        if m_size:
          ...
          results.append(f'tft.setTextColor({int(m_color.group(1))});')
        elif m_wrap:
        ...
        max_chars_per_line = math.floor(SCREEN_SIZE.w / CHAR_SIZE[size].w)
        if len(line) > max_chars_per_line:
          if wrap:
            while len(line) > max_chars_per_line:
              results.append(f'tft.println("{line[:max_chars_per_line]}");')
        ...
        if y_loc > SCREEN_SIZE.h:
          eprint(f'Entry {key} line "{line}" went past bottom of screen.')
          exit(1)
